...
class LayerNormalization(nn.Module):
    def __init__(self, features: int, eps: float = 10**-6) -> None:   # @ features
        super().__init__()
        self.eps = eps
        self.alpha = nn.Parameter(torch.ones(features)) # Multiplied, Parameter because we want it to learn
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Create a vecctor of shape (seq_len, 1)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        """
        Let x = ( 1/(10000^(2i/d_model)) )
        Then log(x) = log((1/(10000^(2i/d_model))) 
                    = -log(10000^(2i/d_model))
                    = (2i/d_model) * -log(10000)
        Now since exp(log(x)) = x,
        we can say that:
        exp(log(x)) = exp((2i/d_model) * -log(10000))
        So we can say that:
        x = exp( (2i/d_model) * -log(10000) )
        """
        # Apply the sin to even positions
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        
        pe = pe.unsqueeze(0) # (1, seq_len, d_model) dim added for batch
...
class ResidualConnection(nn.Module):
    def __init__(self, features: int, dropout: float) -> None:  # @ features
        super().__init__()
        self.dropout = nn.Dropout(dropout)
...
class MultiheadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_k = d_model // h
        self.w_q = nn.Linear(d_model, d_model, bias=False) # Wq @ , bias=False
        self.w_k = nn.Linear(d_model, d_model, bias=False) # Wk @ , bias=False
        self.w_v = nn.Linear(d_model, d_model, bias=False) # Wv @ , bias=False
        self.w_o = nn.Linear(d_model, d_model, bias=False) # Wo @ , bias=False
        self.dropout = nn.Dropout(dropout)
...
class EncoderBlock(nn.Module): # @ features
...
class Encoder(nn.Module):
    def __init__(self, features: int, layers: nn.ModuleList) -> None:  # @ features
        super().__init__()
        self.layers = layers
        self.norm = LayerNormalization(features)
...
class DecoderBlock(nn.Module):# @ features
